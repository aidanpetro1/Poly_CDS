...
> **Implementation note (v1.6.B PR 2): path-discrimination + partial closure.** $\int A$ is constructed with **path-discriminating** morphisms — distinct sequences of token-toggles between two fibers are distinct $\mathcal{H}$-morphisms (not quotiented to canonical sorted-symdiff). This preserves clinical trajectory information that snapshot-only models discard. The catch: full categorical closure under composition is unbounded (round-trip toggles produce arbitrarily long paths). PolyCDS ships with **partial closure**: compositions whose combined $\mathcal{H}$-path length exceeds `intA_depth` (default 2) are silently skipped. The category is closed under bounded compositions; longer ones exist in principle but aren't materialized. For v1.6.B's load-bearing consumers (DDx projection, simulator's single-step dynamics, `validate_grothendieck_pointwise`), partial closure is sufficient — no consumer traverses arbitrary-length compositions. If trajectory analytics later require full closure, lift the bound or use $\mathcal{H}_{\mathrm{quot}}$ (canonical-only paths).
>
> **Implementation note (v1.6.B PR 2): lazy $\beta$-side, eager $\delta$-side.** The fibration admits two equivalent presentations (Grothendieck's theorem): the indexed-category form $A : \mathcal{H}^{\mathrm{op}} \to \mathbf{Bicomod}$ with per-fiber sub-bases $(S_h, P_h)$ ($\beta$-side), and the Grothendieck total space $\int A$ with global $(S, P)$ ($\delta$-side). PolyCDS materializes both — but **eagerly only for $\delta$**. Per-fiber strict sub-Bicomodules ($\beta$-side) are constructed lazily on demand because building $S_h$ via `cached_to_category(S_{\mathrm{joint}})` is the bottleneck (>3 minutes on v1.x's joint comonoids; the joint duplicator's pointwise formula is expensive at scale). v1.6.B's load-bearing consumers (DDx, simulator, runtime checks) use only $\int A$. $\beta$-side materialization is reserved for external composition (v1.8 shared-objective library) and opt-in `validate_grothendieck_pointwise`.
...
> **Note on Friedman's *Fundamental Theorem of Biomedical Informatics* (Aidan, 2026-04-30).** The fiber predicate `is_h_compatible` intentionally cannot distinguish "considered & ruled out" from "never on DDx" at the snapshot level — that distinction lives in the trajectory through $\int A$, not in any single $A_h$. This is a structural reflection of Friedman 2009 ("A 'Fundamental Theorem' of Biomedical Informatics", JAMIA 16(2):169-170): the value of a CDS lies in the *partnership* — the interaction trajectory between clinician and information resource — not in the data snapshot alone. Empty $A_h$ at runtime is itself a meaningful signal: an incoherent $h$ has no compatible positions, and the framework surfaces this rather than masking it. We do not add a `validate_h_coherent` gate that pre-filters incoherent histories; the empty-fiber outcome is the signal.
...
- **v1.6.B PR 1 (pushed 2026-04-30):** H-fibration infrastructure. Problem vocabulary $\Sigma_{\mathrm{prob}}$, ListState as `Set{Symbol}`, $Q = y^{\Sigma_{\mathrm{prob}}}$ toggle reframe (§22), $\sigma$ strict toggle coalgebra, `realize` / `cc_realize` maps for D1/D2, derived $\Theta$ quotient, patient-history category $\mathcal{H}$ with path-discriminating + state-quotient dual views.
- **v1.6.B PR 2 (local 2026-04-30):** Fibration construction. Fiber predicate `is_h_compatible`; $\int A$ SmallCategory ($\delta$-side, eager); per-fiber strict sub-Bicomodule with sub-bases $(S_h, P_h)$ ($\beta$-side, lazy on demand); `FiberedAssessment` struct holding both presentations; reindexing + $\int A$ enumerators; DDx projection (active-workup reading); `cc_fire` H-generator; two-level dynamics in simulator with `:vertical` / `:horizontal` / `:cc_fire` edge-typing; `A_{\mathrm{joint}} \to A_\emptyset$ rename; light `validate_grothendieck_pointwise`. 1251 tests passing.
- **v1.6.B PR 3 (planned):** ProtocolDoc YAML extension (`problem_vocab`, `realize`, `cc_realize` blocks); per-fiber validators (`validate_history_quotient`, `validate_fibration`); foundations-doc deferral entry for the span / comma-category formalization over the considering-token sub-alphabet $\Sigma_{\mathrm{DDx}}$.
